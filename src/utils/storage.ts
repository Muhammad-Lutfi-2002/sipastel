...
// on the order row can be rendered directly as <img src> in the admin
// panel without needing a signed URL round-trip.
...
// Uploads a customer's custom-order design file to Supabase Storage and
// returns its public URL, or null if the upload failed (network issue,
// bucket missing/misconfigured, file too large, etc). Callers should treat
// a null return as non-fatal — the order can still be submitted, just
// without a viewable mockup image, and the customer can send the file via
// WhatsApp as a fallback.
//
// NOTE: A Cloudflare R2-backed version of this function (via a Worker API,
// see worker/ and src/utils/imageUpload.ts) is already built and validated
// in this repo, just not wired in yet. To switch back to R2 later, replace
// this function's body with `uploadImage(file, 'designs', orderId)` from
// './imageUpload', and re-enable "main"/"r2_buckets" in wrangler.jsonc.
...
// SELECT). Generates a simple sequential-looking id/slug consistent with
// the studio's existing "prod-xxxx" seed data.
...
// Used both for the "Edit Product" modal (name/category/price/etc.) and
// implicitly covers price changes, which is the field the customer form
// depends on most (it always reads the live price straight from Supabase).
export async function updateStoredProduct(
  ...
  if (updates.imageUrl !== undefined) payload.images = updates.imageUrl ? [updates.imageUrl] : [];
...
// Stored in the "branding-assets" Supabase Storage bucket (public read,
// admin-only write via RLS) - the same storage system already used for
// custom-order design files, just a separate bucket so branding assets
// don't mix with customer uploads. studio_profile.logo_url is the single
// source of truth for which logo is "active"; every header/navbar reads
// from there, so swapping it here is reflected everywhere with no
// redeploy required.
...
export async function uploadStudioLogo(
  ...
  if (!ALLOWED_LOGO_TYPES.includes(file.type)) {
    return { success: false, error: 'Format tidak didukung. Gunakan PNG, JPG, WEBP, atau SVG.' };
  }
  ...
  try {
    ...
    const saved = await saveStoredStudioProfile({
      ...(await getStoredStudioProfile()),
      logoUrl: data.publicUrl,
    });
    if (!saved) {
      return { success: false, error: 'Logo terupload tapi gagal menyimpan referensinya ke pengaturan studio.' };
    }
...
export async function removeStudioLogo(currentLogoUrl: string | null): Promise<{ success: boolean; error?: string }> {
  try {
    if (currentLogoUrl) {
      const path = currentLogoUrl.split(`/${LOGO_BUCKET}/`).pop();
      if (path) {
        await supabase.storage.from(LOGO_BUCKET).remove([path]);
      }
    }
    const saved = await saveStoredStudioProfile({ ...(await getStoredStudioProfile()), logoUrl: null });
    if (!saved) {
      return { success: false, error: 'Gagal menghapus referensi logo dari pengaturan studio.' };
    }
    return { success: true };
  } catch (e) {
