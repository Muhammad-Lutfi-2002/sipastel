...
export const ProductGrid: React.FC<ProductGridProps> = ({
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-8 sm:mb-12 pb-4 border-b border-[#E8E5DF] gap-3">
        <div>
          ...
          {subtitle && (
            <p className="text-xs sm:text-sm text-[#75726B] mt-1.5 max-w-md">{subtitle}</p>
          )}
        </div>
        ...
        {!isShopPage && onViewAllShop && (
          <button
            ...
            className="text-xs font-bold uppercase tracking-wider text-[#1C1B1A] hover:underline underline-offset-4 self-start sm:self-auto py-1 cursor-pointer"
          >
            Explore All Pieces →
          </button>
        )}
                ...
                <button
                  ...
                  className={`min-h-[40px] px-4 py-2 text-xs font-bold uppercase tracking-wider rounded-[2px] transition-all whitespace-nowrap cursor-pointer ${
                    selectedCategory === cat.key
                      ...
                      : 'bg-[#F2EFE8] text-[#55524B] hover:text-[#1C1B1A] hover:bg-[#EAE5DA]'
                  }`}
                >
                  {cat.label}
                </button>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="relative flex-1 sm:w-64">
                <input
                  ...
                  className="w-full min-h-[42px] pl-9 pr-8 py-2 bg-[#FAF9F5] border border-[#DCD8D0] rounded-[2px] text-xs text-[#1C1B1A] placeholder-[#9E9B93] focus:outline-none focus:border-[#1C1B1A]"
                />
                <Search className="w-3.5 h-3.5 text-[#75726B] absolute left-3 top-3.5" />
                {searchQuery && (
                  <button
                    onClick={() => setSearchQuery('')}
                    className="p-1 text-[#75726B] hover:text-[#1C1B1A] absolute right-2 top-2.5"
              ...
              <div className="relative shrink-0">
                <select
                  ...
                  className="appearance-none min-h-[42px] pl-3.5 pr-8 py-2 bg-[#FAF9F5] border border-[#DCD8D0] rounded-[2px] text-xs font-semibold text-[#1C1B1A] focus:outline-none focus:border-[#1C1B1A] cursor-pointer"
                >
                ...
                <ArrowUpDown className="w-3 h-3 text-[#75726B] absolute right-2.5 top-3.5 pointer-events-none" />
              </div>
            </div>
          </div>
        </div>
      ...
      {isLoading ? (
        ...
        <div id="no-products-state" className="py-16 sm:py-24 text-center bg-[#F4F1EA] rounded-[2px] border border-[#E5E1D7] my-6 animate-fade-in">
          <p className="font-heading text-xl font-bold text-[#1C1B1A]">No products found.</p>
          <p className="text-xs sm:text-sm text-[#66645E] mt-2 max-w-sm mx-auto">
          ...
          <button
            ...
            className="mt-6 min-h-[42px] px-6 py-2.5 bg-[#1C1B1A] hover:bg-[#2C2B29] text-[#FAF9F5] text-xs font-bold uppercase tracking-wider rounded-[2px] transition-all hover:-translate-y-[1px] active:scale-[0.98] cursor-pointer"
          >
