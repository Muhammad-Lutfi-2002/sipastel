  ...
  return (
    ...
    >
      ...
      {showBubble && (
        <div
          id="whatsapp-popover"
          className="bg-[#1C1B1A] text-[#FAF9F5] px-3.5 py-2 rounded-xs shadow-lg text-xs flex items-center gap-2 border border-[#333230] animate-fade-in"
        >
          <span className="leading-tight">Ada pertanyaan? Chat tim SIPASTEL</span>
          <button
            onClick={() => setShowBubble(false)}
            className="text-[#A8A59E] hover:text-white p-0.5"
            aria-label="Tutup"
          >
      ...
      <a
        ...
        className="w-11 h-11 bg-[#1C1B1A] text-[#FAF9F5] hover:bg-[#2C2B29] rounded-full flex items-center justify-center shadow-md border border-[#42403B] transition-transform hover:scale-105 active:scale-95"
        aria-label="Chat WhatsApp SIPASTEL"
        title="Chat WhatsApp SIPASTEL"
      >
        <MessageCircle className="w-5 h-5 text-[#677663]" />
      </a>
    </div>
  );
};
