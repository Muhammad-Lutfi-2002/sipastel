...
export const CustomOrderForm: React.FC<CustomOrderFormProps> = ({
  ...
  const dpAmount =
    formData.paymentPreference === 'DP' ? Math.round((totalPrice * formData.paymentPreferencePercentage) / 100) : totalPrice;
    ...
    return (
      <div id="custom-order-confirmation" className="w-full max-w-3xl mx-auto py-12 px-4 sm:px-6">
        <div className="bg-[#FAF9F5] border border-[#E8E5DF] p-6 sm:p-10 rounded-xs shadow-sm">
          ...
          {orderTotal > 0 && (
            <div className="mt-4 p-5 bg-[#F6F4ED] rounded-xs border border-[#E8E5DF] space-y-3">
              <div className="flex items-center gap-2 pb-2 border-b border-[#E5E1D7]">
              ...
              <div className="space-y-1.5 text-xs sm:text-sm">
                <div className="flex items-center justify-between text-[#55524B]">
                  ...
                  <span className="font-semibold text-[#1C1B1A]">{formatIDR(orderTotal)}</span>
                </div>
                <div className="flex items-center justify-between text-[#55524B]">
                  <span>
                  ...
                  <span className="font-bold text-[#B25D52]">{formatIDR(orderPayNow)}</span>
                </div>
                {submittedOrder.paymentPreference === 'DP' && (
                  <div className="flex items-center justify-between text-[#55524B]">
                    <span>Sisa Pembayaran (Pelunasan)</span>
                    <span className="font-semibold text-[#1C1B1A]">{formatIDR(orderRemaining)}</span>
                  </div>
                )}
              </div>

              {studioProfile?.bankAccountNumber && (
                <div className="pt-3 border-t border-[#E5E1D7] text-xs sm:text-sm">
                  <span className="text-[#75726B] block mb-1">
            ...
            <div className="space-y-6 animate-fade-in">
              ...
              <div>
                <label className="block text-xs font-semibold uppercase tracking-wider text-[#42403B] mb-2.5">
                  Produk <span className="text-[#B25D52]">*</span>
                </label>

                {isLoadingCatalog ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {[1, 2, 3, 4].map((i) => (
                      <div key={i} className="h-20 rounded-xs bg-[#F0ECE1] animate-pulse" />
                    ))}
                  </div>
                ) : catalogProducts.length === 0 ? (
                  <div className="p-4 rounded-xs border border-[#E2DFD8] bg-[#FAF9F5] text-xs text-[#75726B]">
                      ...
                      >
                        {product.images?.[0] && (
                          <img
                            ...
                            alt={product.name}
                            className="w-12 h-12 rounded-xs object-cover border border-[#E2DFD8] shrink-0 bg-white"
                            referrerPolicy="no-referrer"
                          />
                        )}
                        <div className="min-w-0">
                          <p className="font-heading font-bold text-sm text-[#1C1B1A] truncate">
                            {product.name}
                          </p>
                          <p className="text-xs text-[#75726B] truncate">
                            {product.shortDescription || product.category}
                          </p>
                          <p className="text-sm font-bold text-[#B25D52] mt-0.5">
                            {formatIDR(product.price)}{' '}
                            <span className="text-[10px] font-normal text-[#8C8880]">/ pcs</span>
                          </p>
                        </div>
                      </button>
                    ))}
                ...
                {errors.selectedProductId && (
                  <p className="text-xs text-[#B25D52] mt-1">{errors.selectedProductId}</p>
                )}
              </div>

              <div>
                <label className="block text-xs font-semibold uppercase tracking-wider text-[#42403B] mb-1.5">
                  Jumlah / Quantity (Pcs) <span className="text-[#B25D52]">*</span>
                </label>
              ...
              {selectedProduct && (
                <div className="bg-[#F6F4ED] p-4 rounded-xs border border-[#E8E5DF]">
                  <div className="flex items-center justify-between text-xs text-[#55524B]">
                    <span>
                      {selectedProduct.name} — {formatIDR(selectedProduct.price)} × {formData.quantity} pcs
                    </span>
                  </div>
                  <div className="flex items-center justify-between mt-2 pt-2 border-t border-[#E5E1D7]">
                    <span className="text-xs font-bold uppercase tracking-wider text-[#42403B]">
                      Total Harga
                    </span>
                    <span className="font-heading text-lg font-bold text-[#1C1B1A]">
          ...
          {currentStep === 7 && (
            <div className="space-y-6 animate-fade-in">
              ...
              <div className="bg-[#F6F4ED] p-5 rounded-xs border border-[#E8E5DF] space-y-4 text-xs sm:text-sm">
                ...
                {selectedProduct && (
                  <div className="flex items-center justify-between pb-4 border-b border-[#E5E1D7]">
                    <span className="text-xs font-bold uppercase tracking-wider text-[#42403B]">
                      Total Harga
                    </span>
                    <span className="font-heading text-lg font-bold text-[#1C1B1A]">
              ...
              <div className="bg-[#F6F4ED] p-5 rounded-xs border border-[#E8E5DF] space-y-4">
                ...
                <p className="text-xs text-[#75726B] -mt-2">
                  Total harga sudah pasti berdasarkan katalog di atas. Pilih mau bayar DP atau lunas sekarang —
                  pembayaran aktual tetap dikonfirmasi manual oleh admin setelah dana diterima.
                </p>
                ...
                {/* Live-calculated payment breakdown - updates instantly with total/percentage */}
                {selectedProduct && (
                  <div className="pt-3 border-t border-[#E5E1D7] space-y-1.5 text-xs">
                    <div className="flex items-center justify-between text-[#55524B]">
                      <span>Total Harga</span>
                      <span className="font-semibold text-[#1C1B1A]">{formatIDR(totalPrice)}</span>
                    </div>
                    <div className="flex items-center justify-between text-[#55524B]">
                      <span>
                        {formData.paymentPreference === 'DP'
                      ...
                      <span className="font-bold text-[#B25D52]">{formatIDR(dpAmount)}</span>
                    </div>
                    {formData.paymentPreference === 'DP' && (
                      <div className="flex items-center justify-between text-[#55524B]">
                        <span>Sisa Pembayaran (Pelunasan)</span>
                        <span className="font-semibold text-[#1C1B1A]">{formatIDR(remainingAfterDp)}</span>
                      </div>
                    )}
                  </div>
                )}

                {studioProfile?.bankAccountNumber && selectedProduct && (
