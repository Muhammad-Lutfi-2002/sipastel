        ...
        {toasts.map((toast) => {
          ...
          return (
            <div
              key={toast.id}
              role="alert"
              className="pointer-events-auto relative overflow-hidden bg-white border border-[#E8E5DF] rounded-2xl shadow-[0_8px_30px_rgba(28,27,26,0.12)] flex items-start gap-3 p-3.5 pl-4 animate-toast-in"
            >
              ...
              <p className="flex-1 text-xs sm:text-[13px] font-medium text-[#1C1B1A] leading-snug pt-1.5">
                {toast.message}
              </p>
              <button
                onClick={() => dismiss(toast.id)}
                className="text-[#B8B4AA] hover:text-[#1C1B1A] p-1 -m-0.5 mt-1 transition-colors shrink-0 cursor-pointer"
