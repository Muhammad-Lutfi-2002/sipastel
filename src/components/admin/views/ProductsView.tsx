    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-[#E8E5DF]">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="px-3 py-1.5 bg-[#1C1B1A] hover:bg-[#33312E] text-white rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-colors cursor-pointer shadow-xs"
          >
              ...
              {filteredProducts.length === 0 ? (
                ...
                filteredProducts.map((prod) => (
                  <tr key={prod.id} className="hover:bg-[#FAF6F2] transition-colors">
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-3">
                        <img
                          ...
                          alt={prod.name}
                          className="w-10 h-10 rounded-lg object-cover border border-[#E8E5DF] bg-white"
                          referrerPolicy="no-referrer"
                        />
                        <div>
                          <p className="font-semibold text-[#1C1B1A]">{prod.name}</p>
                    ...
                    <td className="py-3 px-4 font-mono font-bold text-[#1C1B1A]">
                    ...
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        {(prod.colors || []).map((c, i) => (
                          <span
                            key={i}
                            className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-[#FAF9F5] border border-[#E8E5DF] text-[10px] text-[#5E5B54]"
                          >
                            <span
                    ...
                    <td className="py-3 px-4 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <button
                          ...
                          className="p-1.5 text-[#8C8880] hover:text-[#1C1B1A] hover:bg-[#F2EFE9] rounded-md transition-colors cursor-pointer"
      ...
      {isFormOpen && (
        <div className="fixed inset-0 z-50 bg-[#1C1B1A]/40 backdrop-blur-xs flex items-center justify-center p-4 animate-modal-backdrop">
          <div className="bg-white rounded-2xl max-w-lg w-full border border-[#E8E5DF] shadow-xl animate-modal-content max-h-[90vh] overflow-y-auto">
            <div className="p-5 sm:p-6 border-b border-[#E8E5DF] flex items-center justify-between">
              <h4 className="font-heading text-base font-bold text-[#1C1B1A]">
                {editingProduct ? `Edit Produk: ${editingProduct.name}` : 'Tambah Produk Baru'}
              </h4>
              <button
                onClick={() => setIsFormOpen(false)}
                className="p-1 text-[#8C8880] hover:text-[#1C1B1A] cursor-pointer"
                aria-label="Tutup"
              >
                <X className="w-4 h-4" />
              </button>
            ...
            <div className="p-5 sm:p-6 space-y-4">
              <div>
                <label className="block text-xs font-semibold text-[#42403B] mb-1.5">
                  Nama Produk <span className="text-[#B84A48]">*</span>
                </label>
                <input
                  type="text"
                  ...
                  className="w-full px-3 py-2 bg-[#FAF9F5] border border-[#E8E5DF] focus:border-[#D87A61] rounded-lg text-sm text-[#1C1B1A] focus:outline-hidden transition-colors"
                />
                {formErrors.name && <p className="text-xs text-[#B84A48] mt-1">{formErrors.name}</p>}
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-[#42403B] mb-1.5">Kategori</label>
                  <select
                ...
                <div>
                  <label className="block text-xs font-semibold text-[#42403B] mb-1.5">
                    Harga / pcs (Rp) <span className="text-[#B84A48]">*</span>
                  </label>
                  <input
                    type="number"
                    min="0"
                    ...
                    className="w-full px-3 py-2 bg-[#FAF9F5] border border-[#E8E5DF] focus:border-[#D87A61] rounded-lg text-sm text-[#1C1B1A] focus:outline-hidden transition-colors"
                  />
                  {formErrors.price && <p className="text-xs text-[#B84A48] mt-1">{formErrors.price}</p>}
                </div>
              </div>

              <div>
                <label className="block text-xs font-semibold text-[#42403B] mb-1.5">Deskripsi Singkat</label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-[#42403B] mb-1.5">Material</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-semibold text-[#42403B] mb-1.5">URL Gambar</label>
                  <input
                    type="text"
              ...
              <label className="flex items-center gap-2 text-xs text-[#42403B] cursor-pointer">
                <input
                  type="checkbox"
                  checked={formValues.inStock}
                  onChange={(e) => setFormValues({ ...formValues, inStock: e.target.checked })}
                  className="w-4 h-4 accent-[#1C1B1A]"
                />
            ...
            <div className="p-5 sm:p-6 pt-0 flex justify-end gap-2">
              ...
              <button
                ...
                className="px-3.5 py-1.5 bg-[#1C1B1A] hover:bg-[#33312E] disabled:opacity-60 text-white text-xs font-semibold rounded-lg cursor-pointer"
              >
      ...
      {productToDelete && (
        <div className="fixed inset-0 z-50 bg-[#1C1B1A]/40 backdrop-blur-xs flex items-center justify-center p-4 animate-modal-backdrop">
          <div className="bg-white p-6 rounded-2xl max-w-sm w-full border border-[#E8E5DF] shadow-xl space-y-4 animate-modal-content">
            <h4 className="font-heading font-bold text-[#1C1B1A]">Hapus Produk</h4>
            <p className="text-xs text-[#5E5B54] leading-relaxed">
              Anda yakin ingin menghapus <strong className="text-[#1C1B1A]">{productToDelete.name}</strong> dari katalog studio?
            </p>
