    ...
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-[#E8E5DF]">
        ...
        <div className="flex items-center gap-2 flex-wrap sm:flex-nowrap">
          ...
          {activeStageFilter === 'ALL' && (
            <div className="flex items-center gap-1 bg-white border border-[#E8E5DF] rounded-lg p-0.5 shadow-2xs">
              <button
                type="button"
                onClick={() => scrollHorizontal('left')}
                className="p-1 text-[#5E5B54] hover:text-[#1C1B1A] hover:bg-[#F4F1EA] rounded-md transition-colors cursor-pointer"
                title="Geser ke kiri"
          ...
          <button
            ...
            className="px-2.5 py-1.5 text-xs font-medium text-[#5E5B54] hover:text-[#1C1B1A] bg-white hover:bg-[#F2EFE9] border border-[#E8E5DF] rounded-lg flex items-center gap-1.5 transition-colors cursor-pointer shadow-2xs"
          >
      ...
      <div className="bg-white border border-[#E8E5DF] rounded-2xl p-3.5 sm:p-4 shadow-[0_4px_20px_rgba(28,27,26,0.06)] overflow-hidden">
        <div className="flex items-center justify-between pb-3 border-b border-[#F0ECE5] text-xs">
          <div className="flex items-center gap-2">
            ...
            <span className="font-mono font-bold uppercase tracking-wider text-[11px] text-[#1C1B1A]">
              Urutan Alur Kerja
            </span>
          ...
          <div className="flex items-center gap-3 font-mono text-[11px] text-[#5E5B54]">
            <span>
              Dalam Alur: <strong className="text-[#1C1B1A]">{totalOrdersInProduction}</strong> pesanan
            </span>
            <span>•</span>
            <span>
              Total Volume: <strong className="text-[#1C1B1A]">{totalGarments}</strong> pcs
            </span>
          </div>
        </div>
              ...
              return (
                <React.Fragment key={st.id}>
                  ...
                  <button
                    ...
                    className="flex-1 min-w-[110px] p-2 rounded-lg border border-[#E8E5DF] bg-[#FAF9F5] hover:bg-white hover:border-[#D87A61]/60 hover:shadow-xs transition-all text-left group cursor-pointer"
                  >
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-[10px] font-mono font-bold text-[#8C8880] group-hover:text-[#C14E30]">
                        TAHAP {st.stepNumber}
                      </span>
                      <span className="w-4 h-4 rounded-full bg-white border border-[#DDD8CD] flex items-center justify-center font-mono font-bold text-[9px] text-[#1C1B1A]">
                        {count}
                      </span>
                    </div>
          ...
          {displayedStages.map((stage, idx) => {
            ...
            return (
              <div
                key={stage.id}
                id={`lane-${stage.id}`}
                className="w-[330px] sm:w-[350px] min-w-[330px] sm:min-w-[350px] shrink-0 bg-[#FAF9F5] border border-[#E8E5DF] rounded-2xl flex flex-col shadow-[0_4px_20px_rgba(28,27,26,0.06)] min-h-[520px] max-h-[720px]"
              >
                {/* Lane Sticky Header */}
                <div className="p-3.5 border-b border-[#E8E5DF] bg-[#FCFAF7] rounded-t-xl space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="w-5 h-5 rounded-full bg-white border border-[#DDD8CD] flex items-center justify-center font-mono font-bold text-[10px] text-[#1C1B1A]">
                        {stageOrders.length}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    <div className="w-7 h-7 rounded-lg bg-white border border-[#E8E5DF] flex items-center justify-center text-[#5E5B54] shadow-2xs">
                      <StageIcon className="w-4 h-4" />
                    </div>
                    <div>
                      <h3 className="font-heading text-sm font-bold text-[#1C1B1A] leading-tight">
                        {stage.title}
                      </h3>
                ...
                <div className="flex-1 p-3 space-y-2.5 overflow-y-auto">
                  {stageOrders.length === 0 ? (
                    ...
                    stageOrders.map((order) => (
                      <div
                        key={order.orderId}
                        className="p-3 bg-white border border-[#E8E5DF] hover:border-[#D87A61]/60 rounded-lg shadow-2xs transition-all space-y-2.5 cursor-pointer group focus:outline-none focus-visible:ring-2 focus-visible:ring-[#D87A61]"
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between">
                          <span className="font-mono font-bold text-xs text-[#1C1B1A] group-hover:text-[#C14E30] transition-colors">
                            {order.orderId}
                          </span>
                          <div className="flex items-center gap-1.5">
                            <span
                        ...
                        <div>
                          <p className="text-xs font-semibold text-[#1C1B1A] flex items-center justify-between">
                            <span>{order.customer}</span>
                            <span className="font-mono font-bold text-xs text-[#1C1B1A]">
                              {order.quantity} pcs
                            </span>
                          </p>
                        ...
                        <div className="pt-2 border-t border-[#F2EFE9] flex items-center justify-between gap-2">
                          <button
                            ...
                            className="text-[11px] font-medium text-[#5E5B54] hover:text-[#1C1B1A] flex items-center gap-1 cursor-pointer"
                          >
                            <span>Periksa</span>
                ...
                {/* Lane Footer */}
                <div className="p-2 border-t border-[#E8E5DF] bg-[#FAF9F5] rounded-b-xl text-center">
                  <span className="text-[10px] font-mono text-[#8C8880]">
