...
export const OrderDetailView: React.FC<OrderDetailViewProps> = ({ orderId }) => {
  ...
  if (isLoading) {
    return (
      <div className="p-8 space-y-4 bg-white border border-[#EAE6DF] rounded-2xl my-6">
  ...
  if (!order) {
    return (
      <div className="p-8 text-center space-y-4 bg-white border border-[#EAE6DF] rounded-2xl my-6">
        <h2 className="text-base font-bold text-[#1F1E1D]">Pesanan Tidak Ditemukan</h2>
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-6xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <button
          onClick={() => navigate('/admin/orders')}
          className="inline-flex items-center gap-1.5 text-xs text-[#5E5B54] hover:text-[#1F1E1D] transition-colors cursor-pointer"
        >
      ...
      <div className="p-4 sm:p-6 bg-white border border-[#EAE6DF] rounded-2xl shadow-[0_4px_20px_rgba(28,27,26,0.06)] flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 flex-wrap">
            <span className="font-mono text-lg sm:text-xl font-bold text-[#1F1E1D]">
              {order.orderId}
            </span>
            <button
              type="button"
              onClick={handleCopyOrderId}
              className="p-1 text-[#8C8880] hover:text-[#1F1E1D] hover:bg-[#F2EFE9] rounded transition-colors cursor-pointer"
          ...
          <p className="text-xs sm:text-sm text-[#5E5B54] mt-1">
            Pelanggan: <span className="font-semibold text-[#1F1E1D]">{order.customer}</span> •{' '}
            {order.phone}
          </p>
        </div>
      ...
      {order.isCustomOrder && (
        <div className="p-4 sm:p-6 bg-white border border-[#EAE6DF] rounded-2xl space-y-4 shadow-[0_4px_20px_rgba(28,27,26,0.06)]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-5 items-center">
            {/* Visual Mockup Box */}
            <div className="md:col-span-4 bg-[#FAF9F5] border border-[#E8E4DA] rounded-lg p-3 flex flex-col items-center justify-center text-center">
              {order.design ? (
                <div
                  className="relative group cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-[#1F1E1D] rounded"
                ...
                >
                  <img
                    ...
                    className="max-h-52 w-auto object-contain rounded border border-[#E8E4DA] bg-white shadow-2xs"
            ...
            <div className="md:col-span-8 space-y-3.5">
              <div className="space-y-1">
                ...
                <p className="text-xs font-mono font-semibold text-[#1F1E1D]">
                ...
                <p className="text-xs text-[#5E5B54]">
                  ...
                  <strong className="text-[#1F1E1D]">{order.productType || 'Jersey Custom'}</strong>
                </p>
              </div>

              {order.notes && (
                <div className="p-3 bg-[#FAF9F5] border border-[#E8E4DA] rounded-lg text-xs space-y-1">
                  <span className="font-semibold text-[#1F1E1D] block">Catatan Penempatan Pelanggan:</span>
                ...
                {order.design && (
                  <>
                    ...
                    <a
                      ...
                      className="px-3 py-1.5 bg-white hover:bg-[#F2EFE9] border border-[#E8E4DA] text-xs font-medium text-[#1F1E1D] rounded-lg flex items-center gap-1.5 transition-colors"
                    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="p-4 sm:p-5 bg-white border border-[#EAE6DF] rounded-2xl space-y-4 shadow-[0_4px_20px_rgba(28,27,26,0.06)]">
            <div className="flex items-center justify-between border-b border-[#F0ECE5] pb-3">
              <h3 className="font-heading text-sm font-bold text-[#1F1E1D]">
                Item &amp; Spesifikasi
              </h3>
            ...
            <div className="space-y-3 divide-y divide-[#F2EFE9]">
              {(order.items || []).map((item, idx) => (
                <div key={idx} className="pt-3 first:pt-0 flex items-start justify-between gap-3 text-xs">
                  <div>
                    <p className="font-semibold text-[#1F1E1D] text-sm">{item.productName}</p>
                    <div className="text-[#5E5B54] mt-1 space-y-0.5">
                      {item.size && (
                        <p>
                          Ukuran: <span className="font-mono font-semibold text-[#1F1E1D]">{item.size}</span>
                        </p>
                      )}
                      {item.variantColor && (
                        <p>
                          Warna: <span className="text-[#1F1E1D]">{item.variantColor}</span>
                        </p>
                      )}
                      {order.material && (
                        <p>
                          Bahan: <span className="text-[#1F1E1D]">{order.material}</span>
                        </p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="text-right">
                    ...
                    {item.price && (
                      <p className="font-mono font-bold text-[#1F1E1D] mt-0.5">
            ...
            {/* Total summary */}
            <div className="pt-3 border-t border-[#EAE6DF] flex items-center justify-between text-xs font-medium">
              ...
              {isEditingPrice ? (
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="w-32 p-1.5 bg-[#FAF9F5] border border-[#E8E4DA] rounded-lg text-xs font-mono focus:outline-hidden"
                  />
                  <button
                    type="button"
                    onClick={handleSavePrice}
                    disabled={isSavingPrice}
                    className="px-2.5 py-1.5 bg-[#1F1E1D] hover:bg-[#33312E] disabled:opacity-50 text-white text-[11px] font-semibold rounded-lg cursor-pointer"
                  >
                  ...
                  <button
                    ...
                    className="px-2.5 py-1.5 border border-[#E8E4DA] text-[#5E5B54] text-[11px] font-semibold rounded-lg cursor-pointer"
                  >
                    Batal
                  </button>
                </div>
          ...
          <div className="p-4 sm:p-5 bg-white border border-[#EAE6DF] rounded-2xl space-y-3.5 shadow-[0_4px_20px_rgba(28,27,26,0.06)] text-xs">
            <h3 className="font-heading text-sm font-bold text-[#1F1E1D] border-b border-[#F0ECE5] pb-2.5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1">
                ...
                <p className="text-[#1F1E1D] leading-relaxed">{order.address}</p>
            ...
            <div className="pt-3 border-t border-[#F0ECE5] flex items-center justify-between text-[11px]">
              ...
              <span className="font-mono text-[#5E5B54]">
                Resi / Tracking:{' '}
                <strong className="text-[#1F1E1D]">{order.trackingNumber || 'Belum ada'}</strong>
              </span>
            </div>
          </div>
          ...
          {order.paymentPreference && (
            <div className="p-4 bg-[#F6F4ED] border border-[#E8E5DF] rounded-2xl text-xs">
              ...
              <p className="font-bold text-[#1F1E1D]">
        ...
        <div className="lg:col-span-5 space-y-6">
            ...
            <form onSubmit={handleSaveStatus} className="space-y-3.5 text-xs">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full p-2 bg-[#FAF9F5] border border-[#E8E4DA] rounded-lg text-xs font-medium text-[#1F1E1D] focus:outline-hidden"
                >
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full p-2 bg-[#FAF9F5] border border-[#E8E4DA] rounded-lg text-xs font-mono text-[#1F1E1D] focus:outline-hidden"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full p-2 bg-[#FAF9F5] border border-[#E8E4DA] rounded-lg text-xs text-[#1F1E1D] focus:outline-hidden resize-none"
                />
              </div>

              <button
                type="submit"
                className="w-full py-2 bg-[#1F1E1D] hover:bg-[#33312E] text-white rounded-lg text-xs font-semibold flex items-center justify-center gap-1.5 transition-colors cursor-pointer shadow-xs"
              >
          ...
          <div className="p-4 sm:p-5 bg-white border border-[#EAE6DF] rounded-2xl space-y-3 shadow-[0_4px_20px_rgba(28,27,26,0.06)]">
            <h3 className="font-heading text-sm font-bold text-[#1F1E1D] border-b border-[#F0ECE5] pb-2.5">
                ...
                order.timelineLogs.map((log) => (
                  <div key={log.id} className="pt-2.5 first:pt-0 space-y-0.5">
                    <div className="flex items-center justify-between text-[11px]">
                      <span className="font-semibold text-[#1F1E1D] font-mono">
      ...
      {isPreviewModalOpen && order.design && (
        ...
        >
          <div
            className="bg-white p-4 sm:p-6 rounded-2xl max-w-3xl w-full border border-[#EAE6DF] shadow-2xl space-y-4 animate-modal-content"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between border-b border-[#EAE6DF] pb-3">
              <div>
                <h4 className="font-heading text-sm font-bold text-[#1F1E1D]">
              ...
              <button
                ...
                className="p-1 text-[#8C8880] hover:text-[#1F1E1D] rounded-lg cursor-pointer"
              >
                <X className="w-5 h-5" />
              </button>
            </div>

            <div className="bg-[#FAF9F5] border border-[#EAE6DF] rounded-lg p-4 flex items-center justify-center max-h-[70vh] overflow-auto">
              <img
                src={order.design}
            ...
            <div className="flex justify-end gap-2 pt-2 border-t border-[#EAE6DF]">
              <a
                ...
                className="px-3.5 py-1.5 bg-[#1F1E1D] text-white text-xs font-semibold rounded-lg flex items-center gap-1.5 transition-colors cursor-pointer"
              >
