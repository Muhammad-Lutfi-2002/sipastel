...
import { getStoredOrders, updateOrderDetails } from '../../../utils/storage';
...
export const OrderDetailView: React.FC<OrderDetailViewProps> = ({ orderId }) => {
  ...
  const whatsAppMessage = `Halo ${order.customer}, kami dari SIPASTEL Atelier mengenai pesanan Anda (${order.orderId}). Status saat ini: ${order.productionStatus.replace(/_/g, ' ')}. Apakah ada yang bisa kami bantu?`;
          ...
          <div className="p-4 sm:p-5 bg-white border border-[#EAE6DF] rounded-xl space-y-4 shadow-[0_1px_2px_rgba(0,0,0,0.02)]">
            ...
            <div className="pt-3 border-t border-[#EAE6DF] flex items-center justify-between text-xs font-medium">
              ...
              <span className="font-mono text-base font-bold text-[#1F1E1D]">
                {order.totalPrice ? formatIDR(order.totalPrice) : 'Dihitung setelah disetujui'}
              </span>
            </div>
          </div>
