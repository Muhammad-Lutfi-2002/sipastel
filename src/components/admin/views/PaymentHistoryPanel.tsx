...
export const PaymentHistoryPanel: React.FC<PaymentHistoryPanelProps> = ({ order, onPaymentChange }) => {
  ...
  return (
    <div className="bg-white border border-[#EAE6DF] rounded-xl p-5 space-y-5">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <CreditCard className="w-4 h-4 text-[#5E5B54]" />
          <h3 className="font-heading text-sm font-bold text-[#1F1E1D]">Riwayat Pembayaran</h3>
        </div>
        {canRecordPayment && (
          <button
            type="button"
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 bg-[#1F1E1D] hover:bg-[#33312E] disabled:opacity-40 disabled:cursor-not-allowed text-white text-[11px] font-semibold rounded-lg transition-colors cursor-pointer"
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
        <div className="p-3 bg-[#FAF9F5] border border-[#EAE6DF] rounded-lg">
          <p className="text-[10px] font-mono uppercase text-[#8C8880] mb-0.5">Total Tagihan</p>
          <p className="text-sm font-bold text-[#1F1E1D]">{formatIDR(totalPrice)}</p>
        </div>
        ...
        <div className="p-3 bg-[#FAF9F5] border border-[#EAE6DF] rounded-lg">
          ...
          <div className="flex items-center gap-2">
            <p className="text-sm font-bold text-[#1F1E1D]">{order.paymentPercentage.toFixed(0)}%</p>
      ...
      {isLoadingHistory ? (
        ...
        <div className="text-center py-8 text-xs text-[#8C8880]">
          Belum ada pembayaran tercatat untuk order ini.
        </div>
      ) : (
        <ol className="relative border-l border-[#EAE6DF] ml-2 space-y-5">
          {history.map((p) => (
            <li key={p.paymentId} className="ml-4">
              ...
              />
              <div className={`p-3 rounded-lg border ${p.status === 'VOID' ? 'bg-[#FAF0F0] border-[#F3D4CF] opacity-70' : 'bg-[#FAF9F5] border-[#EAE6DF]'}`}>
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-sm font-bold ${p.status === 'VOID' ? 'line-through text-[#8C2927]' : 'text-[#1F1E1D]'}`}>
                      {formatIDR(p.amount)}
                    </span>
                    {p.status === 'VOID' && (
      ...
      {isFormOpen && (
        ...
        >
          <form
            ...
            className="bg-white rounded-xl p-5 w-full max-w-md space-y-4 shadow-2xl"
          >
            <div className="flex items-center justify-between">
              <h4 className="font-heading text-sm font-bold text-[#1F1E1D]">Catat Pembayaran Baru</h4>
              <button
                type="button"
                onClick={() => setIsFormOpen(false)}
                aria-label="Tutup"
                className="text-[#8C8880] hover:text-[#1F1E1D] cursor-pointer"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                className={`py-2 rounded-lg text-xs font-semibold border cursor-pointer ${
                  dpMode === '20' ? 'bg-[#1F1E1D] text-white border-[#1F1E1D]' : 'bg-[#FAF9F5] border-[#E8E4DA] text-[#1F1E1D]'
                }`}
              >
              ...
              <button
                ...
                className={`py-2 rounded-lg text-xs font-semibold border cursor-pointer ${
                  dpMode === '30' ? 'bg-[#1F1E1D] text-white border-[#1F1E1D]' : 'bg-[#FAF9F5] border-[#E8E4DA] text-[#1F1E1D]'
                }`}
              >
              ...
              <button
                ...
                className={`py-2 rounded-lg text-xs font-semibold border cursor-pointer ${
                  dpMode === 'PELUNASAN' ? 'bg-[#1F1E1D] text-white border-[#1F1E1D]' : 'bg-[#FAF9F5] border-[#E8E4DA] text-[#1F1E1D]'
                }`}
              >
              ...
              <button
                ...
                className={`py-2 rounded-lg text-xs font-semibold border cursor-pointer ${
                  dpMode === 'CUSTOM' ? 'bg-[#1F1E1D] text-white border-[#1F1E1D]' : 'bg-[#FAF9F5] border-[#E8E4DA] text-[#1F1E1D]'
                }`}
              >
                Nominal Manual
              </button>
            ...
            {dpMode === 'CUSTOM' && (
              <div>
                ...
                <input
                  ...
                  className="w-full p-2.5 bg-[#FAF9F5] border border-[#E8E4DA] rounded-lg text-sm focus:outline-hidden"
                />
            ...
            <div>
              ...
              <textarea
                ...
                rows={2}
                className="w-full p-2.5 bg-[#FAF9F5] border border-[#E8E4DA] rounded-lg text-sm focus:outline-hidden resize-none"
            ...
            <div className="p-2.5 bg-[#F4F1EA] rounded-lg text-xs text-[#5E5B54] flex items-center justify-between">
              ...
              <span className="font-bold text-[#1F1E1D]">{formatIDR(computedAmount)}</span>
            </div>
            ...
            <button
              ...
              className="w-full py-2.5 bg-[#1F1E1D] hover:bg-[#33312E] disabled:opacity-50 text-white text-xs font-bold uppercase tracking-wide rounded-lg cursor-pointer flex items-center justify-center gap-1.5"
            >
      ...
      {voidTarget && (
        ...
        >
          <div onClick={(e) => e.stopPropagation()} className="bg-white rounded-xl p-5 w-full max-w-md space-y-4 shadow-2xl">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 py-2.5 border border-[#E8E4DA] text-xs font-semibold rounded-lg cursor-pointer"
              >
                Batal
              </button>
              <button
