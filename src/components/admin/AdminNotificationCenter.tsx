...
export const AdminNotificationCenter: React.FC = () => {
  const [notifications, setNotifications] = useState<AdminNotification[]>(() => {
    ...
    // Ensure all 6 required categories are showcased nicely if only older ones exist
    const hasQC = list.some((n) => n.category === 'QC');
    const hasShipping = list.some((n) => n.category === 'SHIPPING');
    if (!hasQC || !hasShipping) {
      const enriched: AdminNotification[] = [
        ...list,
        ...(!hasQC
          ? [
              {
                id: 'notif-005',
                title: 'Lolos Quality Control',
                message: 'Batch custom jersey SPS-20260905-182 lolos semua pemeriksaan ukuran & ketegangan jahitan.',
          ...
          ? [
              {
                ...
                message: 'Pesanan SPS-20260904-442 sudah dikemas dalam box studio, menunggu pengambilan JNE Express.',
                category: 'SHIPPING' as const,
                orderId: 'SPS-20260904-442',
  ...
  const handleMarkAllRead = () => {
    ...
    setNotifications(updated.map((n) => ({ ...n, isRead: true })));
  };

  const handleClickItem = (notif: AdminNotification) => {
    if (!notif.isRead) {
