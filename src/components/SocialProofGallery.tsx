...
export const SocialProofGallery: React.FC = () => {
  ...
  return (
    <section id="social-proof-gallery-section" className="w-full max-w-7xl mx-auto px-4 sm:px-6 md:px-8 py-12 sm:py-16">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-8 pb-4 border-b border-[#E8E5DF] gap-3">
        <div>
          <span className="text-xs uppercase tracking-widest text-[#75726B] font-semibold block">
            Community & Stories
          </span>
          <h2 className="font-heading text-2xl sm:text-3xl md:text-4xl font-bold tracking-tight text-[#1C1B1A] mt-1">
        ...
        <a
          ...
          className="inline-flex items-center gap-1.5 text-xs font-semibold text-[#1C1B1A] hover:underline underline-offset-4"
        >
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4">
        {photos.map((item, idx) => (
          <div
            key={idx}
            className="group relative overflow-hidden rounded-[2px] bg-[#EFECE4] border border-[#E5E1D7] aspect-[3/4]"
          >
            <LazyImage
              src={item.url}
              alt={item.tag}
            ...
            <div className="absolute inset-0 bg-[#1A1918]/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-3.5 text-[#FAF9F5] z-10">
